...
function App() {
  ...
  // const GetUsers = async () => {
    ...
    // let high = parseFloat(values.high);
    // let low = parseFloat(values.low);
    // let close = parseFloat(values.close);
    const woodiePivot = () =>{
    ...
    const camarillaPivot = () =>{
      let r4 = close + ((high - low)* 1.5000)
      let r3 = close + ((high - low)* 1.2500)
      let r2 = close + ((high - low)* 1.1666)
      let r1 = close + ((high - low)* 1.0833)
      let pp = (high + low + close) / 3;
      let s1 = close - ((high - low)* 1.0833)
      let s2 = close - ((high - low)* 1.1666)
      let s3 = close - ((high - low)* 1.2500)
      let s4 = close - ((high - low)* 1.5000)
    ...
    const fibonacciPivot = ()=> {
      let pp = (high + low + close) / 3;
      let r3 = pp + ((high - low)* 1.000)
      let r2 = pp + ((high - low)* .618)
      let r1 = pp + ((high - low)* .382)

      let s1 = pp - ((high - low)* .382)
      let s2 = pp - ((high - low)* .618)
      let s3 = pp - ((high - low)* 1.000)
      console.log(s2);
